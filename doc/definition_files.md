...
objects, one for each holiday. Individual holidays can be defined as a specific day of the year 
which is appropriate for holidays such as New Years Day and Valentine's Day. 
The day of the year is formatted as 'MMDD' and must appear in quotes to be interpreted as a string. 
Alternatively, a holiday can be defined in a rule (see [dateutil.rrule](https://dateutil.readthedocs.io/en/stable/rrule.html)). A rule
...
| holidays.<*name*>.rrule.interval        | Integer                     | If present, it is the interval of *frequency* at which this holiday occurs, e.g. if *frequency* is **YEARLY** and *interval* is 2 the holiday will occur once every two years (see example below).                                                                                                                                                                                                                                            |
| holidays.<*name*>.rrule.mod             | Integer                     | If present, the result of *frequency* % *interval* at which this holiday occurs, e.g. if *frequency* is **YEARLY**, *interval* is 2, *mod* is 0 the holiday will occur on even years. If *mod* is 1 the holiday will occur on odd years (see example below).                                                                                                                                                                                  |
...
| holidays.<*name*>.rrule.day_of_week     | String or list of strings   | If present, must be one of (**MO**, **TU**, **WE**, **TH**, **FR**, **SA**, **SU**). Each day of the week can, optionally, be followed by an integer argument enclosed in parentheses, e.g. TU(2). The value is the occurrence of this weekday in the period. For example, with **MONTHLY**, or with **YEARLY** and *month* provided, using TU(2) in *day_of_week* will specify the second Tuesday of the month where the recurrence happens. |
